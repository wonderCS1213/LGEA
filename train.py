...
os.environ['CUDA_VISIBLE_DEVICES'] = "0"  # （代表仅使用第2，3号GPU）
...
from src.model import LGEA
from src.utils1 import scipy_sparse_mat_to_torch_sparse_tensor
...
parser.add_argument("--model", type=str, default='lgea', help="the model's name", choices=['gsea', 'gaea', 'gat', 'gcn', 'lgea'])
...
parser.add_argument("--fold", type=int, default=3, help="the fold cross-validation")
...
parser.add_argument('--neg_samples_size', type=int, default=25, help="number of negative samples")
...
parser.add_argument('--ent_dim', type=int, default=512, help="hidden dimension of entity embeddings")
parser.add_argument('--rel_dim', type=int, default=32, help="hidden dimension of relation embeddings")
parser.add_argument('--dropout', type=float, default=0.2, help="dropout rate")
parser.add_argument('--layer', type=int, default=3, help="layer number of GNN-based encoder")
parser.add_argument('--n_head', type=int, default=1, help="number of multi-head attention")
parser.add_argument('--aug_balance', type=float, default=200, help='the hyper-parameter of consistency loss')
...
svd_q = args.q
adj1 = scipy_sparse_mat_to_torch_sparse_tensor(kgdata.adj1.tocoo()).coalesce().cuda(torch.device(device))
svd_u1, s1, svd_v1 = torch.svd_lowrank(adj1, q=svd_q)  # q:rank value
u_mul_s1 = svd_u1 @ (torch.diag(s1))
del s1

adj2 = scipy_sparse_mat_to_torch_sparse_tensor(kgdata.adj2.tocoo()).coalesce().cuda(torch.device(device))
svd_u2, s2, svd_v2 = torch.svd_lowrank(adj2, q=svd_q)
u_mul_s2 = svd_u2 @ (torch.diag(s2))
del s2


rel_in_adj1 = scipy_sparse_mat_to_torch_sparse_tensor(kgdata.rel_adj1[0].tocoo()).coalesce().cuda(torch.device(device))
svd_u1_ri, s1_ri, svd_v1_ri = torch.svd_lowrank(rel_in_adj1, q=svd_q)  # q:rank value
u_mul_s1_ri = svd_u1_ri @ (torch.diag(s1_ri))
del s1_ri
rel_out_adj1 = scipy_sparse_mat_to_torch_sparse_tensor(kgdata.rel_adj1[1].tocoo()).coalesce().cuda(torch.device(device))
svd_u1_ro, s1_ro, svd_v1_ro = torch.svd_lowrank(rel_out_adj1, q=svd_q)  # q:rank value
u_mul_s1_ro = svd_u1_ro @ (torch.diag(s1_ro))
del s1_ro

rel_in_adj2 = scipy_sparse_mat_to_torch_sparse_tensor(kgdata.rel_adj2[0].tocoo()).coalesce().cuda(torch.device(device))
svd_u2_ri, s2_ri, svd_v2_ri = torch.svd_lowrank(rel_in_adj2, q=svd_q)  # q:rank value
u_mul_s2_ri = svd_u2_ri @ (torch.diag(s2_ri))
del s2_ri
rel_out_adj2 = scipy_sparse_mat_to_torch_sparse_tensor(kgdata.rel_adj2[1].tocoo()).coalesce().cuda(torch.device(device))
svd_u2_ro, s2_ro, svd_v2_ro = torch.svd_lowrank(rel_out_adj2, q=svd_q)  # q:rank value
u_mul_s2_ro = svd_u2_ro @ (torch.diag(s2_ro))
del s2_ro
...
elif args.model == "lgea":
    model = LGEA(num_sr=kgdata.kg1_ent_num, num_tg=kgdata.kg2_ent_num, adj_sr=kgdata.tensor_adj1, adj_tg=kgdata.tensor_adj2,
                 u_mul_s_sr=u_mul_s1, vt_sr=svd_v1.T, u_mul_s_tg=u_mul_s2, vt_tg=svd_v2.T,
                 rel_num=kgdata.rel_num, rel_adj_sr=kgdata.tensor_rel_adj1, rel_adj_tg=kgdata.tensor_rel_adj2, device=device, args=args)
...
def evaluate(test_pair, k, eval_metric, phase="test", model_path=None):
    ...
    sr_embedding, tg_embedding, aug_sr_embedding, aug_tg_embedding = model(phase="eval")
...
try:
    ...
    #     aug_adj1_tmp, aug_rel_adj1 = kgdata.generate_aug_graph(kgdata.triples1, kgdata.kg1_ent_num, kgdata.rel_num,
    #                                                            kgdata.kg1_ent_ids, kgdata.ent2node1, kgdata.d_v1,
    #                                                            pr=pr1)
    #     aug_adj2_tmp, aug_rel_adj2 = kgdata.generate_aug_graph(kgdata.triples2, kgdata.kg2_ent_num, kgdata.rel_num,
    ...
    for e in range(args.epoch):
        ...
        for _, data in enumerate(train_loader):
            ...
            if args.model == "gaea":
                ...
                sr_embedding, tg_embedding = model(input1=kgdata.tensor_adj1, input2=kgdata.tensor_adj2, phase="aug")
                loss = loss_fn(sr_embedding, tg_embedding, a1_align, a2_align, neg1_left, neg1_right, neg2_left, neg2_right, neg_samples_size=args.neg_samples_size, loss_norm=args.loss_norm, pos_margin=args.pos_margin, neg_margin=args.neg_margin, neg_param=args.neg_param)
            elif args.model in ["lgea"]:
                # (sr_embedding, tg_embedding, aug_sr_embedding, aug_tg_embedding) = model(aug_rel_adj1, aug_rel_adj2, phase="augment")
                (sr_embedding, tg_embedding, aug_sr_embedding, aug_tg_embedding) = model(u_mul_s1_ri=u_mul_s1_ri,
                                                             svd_v1_ri_t=svd_v1_ri.T,
                                                             u_mul_s1_ro=u_mul_s1_ro,
                                                             svd_v1_ro_t=svd_v1_ro.T,
                                                             u_mul_s2_ri=u_mul_s2_ri,
                                                             svd_v2_ri_t=svd_v2_ri.T,
                                                             u_mul_s2_ro=u_mul_s2_ro,
                                                             svd_v2_ro_t=svd_v2_ro.T, phase="augment")
        ...
        # flag = 0
        # aug_rel_adj1_temp, aug_rel_adj2_temp = aug_rel_adj1, aug_rel_adj2
        # if (e + 1) % args.aug_iter == 0 and e > 0 and args.pr != 0 and e + 1 != args.epoch:
        #     flag = 1
        #     pr1, pr2 = random.uniform(0, args.pr), random.uniform(0, args.pr)
        #     aug_adj1_tmp, aug_rel_adj1_temp = kgdata.generate_aug_graph(kgdata.triples1, kgdata.kg1_ent_num, kgdata.rel_num,
        #                                                        kgdata.kg1_ent_ids, kgdata.ent2node1, kgdata.d_v1,
        #                                                        pr=pr1)
        #     aug_adj2_tmp, aug_rel_adj2_temp = kgdata.generate_aug_graph(kgdata.triples2, kgdata.kg2_ent_num, kgdata.rel_num,
        #                                                        kgdata.kg2_ent_ids, kgdata.ent2node2, kgdata.d_v2,
        #                                                        pr=pr2)
